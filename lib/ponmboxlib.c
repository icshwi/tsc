...
int pop_mbox_data(int *offset, unsigned int *destination, int data_size);
...
int set_mbox_byte(int offset, unsigned char byte);
int push_mbox_data(int *offset, unsigned int byte, int data_size);
int push_mbox_byte(int *offset, unsigned char byte);
int push_mbox_short(int *offset, unsigned short data);
int push_mbox_int(int *offset, unsigned int data);
...
int pop_mbox_data(int *offset, unsigned int *destination, int data_size)
{
  ...
  while (data_size--)
  {
    retval = get_mbox_byte((*offset), &byte);
...
{
  unsigned int value;
  int retval = pop_mbox_data(offset, &value, 1);
...
{
  unsigned int value;
  int retval = pop_mbox_data(offset, &value, 2);
...
int pop_mbox_tribyte(int *offset, unsigned int *destination)
{
  return pop_mbox_data(offset, destination, 3);
}
...
int pop_mbox_int(int *offset, unsigned int *destination)
{
  return pop_mbox_data(offset, destination, 4);
}
...
int push_mbox_data(int *offset, unsigned int data, int data_size)
{
  if (data_size > sizeof(unsigned int)) return EINVAL;
  ...
  while (data_size--)
  {
    retval = set_mbox_byte(*offset, bytes[loop++]);
    if (retval) return retval;
    (*offset)++;
  }
...
int push_mbox_byte(int *offset, unsigned char data)
{
  return push_mbox_data(offset, (unsigned int)data, 1);
}
...
int push_mbox_short(int *offset, unsigned short data)
{
  return push_mbox_data(offset, (unsigned int)data, 2);
}
...
int push_mbox_int(int *offset, unsigned int data)
{
  return push_mbox_data(offset, data, 4);
}
...
int push_mbox_string(int *offset, unsigned char *string)
{
  int retval;

  do
  {
    retval = push_mbox_byte(offset, *string);
    if (retval) return retval;
  }
  while(*(string++));
...
unsigned char create_payload_sensor(mbox_info_t *info, unsigned char *name, int value_size, payload_sensor_handle_t **handle)
{
  if (!info) return EFAULT;
  ...
  int offset;
  if (skip_existing_mbox_sensors(info, &offset)) return EFAULT;
  (*handle) = create_payload_sensor_handle(offset, value_size, name);

  push_mbox_byte(&offset, DESCRIPTOR_TYPE_SENSOR_DATA_VALUE);
  push_mbox_byte(&offset, DESCRIPTOR_FORMAT_VERSION_SENSOR_DATA_VALUE);
  push_mbox_byte(&offset, SENSOR_VALUE_STATUS_NOT_INITIALIZED);
  push_mbox_int(&offset, 0x00); // timestamp
  push_mbox_byte(&offset, value_size);
  push_mbox_data(&offset, 0x00, value_size); // value
  push_mbox_string(&offset, name);
  push_mbox_byte(&offset, DESCRIPTOR_TYPE_NONE); // end of descriptor
...
unsigned char set_payload_sensor_value(payload_sensor_handle_t *handle, int value)
{
  ...
  int value_offset = handle->descriptor_offset + PAYLOAD_SENSOR_DESCRIPTOR_VALUE_OFFSET;
  retval = push_mbox_data(&value_offset, value, handle->value_size);
  if (retval) return retval;
...
unsigned char get_payload_sensors(mbox_info_t *info, payload_sensor_handle_t **handle)
{
  ...
  while (continue_parsing)
  {
    ...
    case DESCRIPTOR_TYPE_SENSOR_DATA_VALUE:
      get_mbox_byte(offset - 2 + PAYLOAD_SENSOR_DESCRIPTOR_VALUE_SIZE_OFFSET, &value_size);
      name_offset = offset - 2 + descriptor_name_offset_for_value_size(value_size);
      pop_mbox_string(&name_offset, &name);
...
unsigned char skip_existing_mbox_sensors(mbox_info_t *info, int *offset)
{
  ...
  while (continue_parsing)
  {
    pop_mbox_byte(&running_offset, &descriptor_type);
    pop_mbox_byte(&running_offset, &descriptor_format_version);
    ...
    case DESCRIPTOR_TYPE_SENSOR_DATA_VALUE:
      get_mbox_byte(running_offset - 2 + PAYLOAD_SENSOR_DESCRIPTOR_VALUE_SIZE_OFFSET, &value_size);
      running_offset = running_offset - 2 + descriptor_name_offset_for_value_size(value_size);
      skip_c_string(&running_offset);
      break;
