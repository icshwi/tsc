...
// --- EXTENSION ---
			...
			if((!strcmp("present", c->para[1])) && (c->cnt == 2)){
				retval = rsp1461_extension_presence(tsc_fd, &present);
				if (present){
					printf("NO \n");
				}
				else {
			...
			if((!strcmp("status", c->para[1])) && (c->cnt == 2)){
				...
				if (present){
					...
					for (i = 0; i < 7; i++){
						retval = rsp1461_extension_get_pin_state(tsc_fd, i, &ext_state, &direction);
						printf("|------+-----------+-------| \n");
						if(direction == 1){
							printf("| [%x]  | in        | %x     | \n", i, ext_state);
						}
			...
			if (c->cnt == 3) {
				...
				if(!strcmp("get", c->para[1])){
					retval = rsp1461_extension_get_pin_state(tsc_fd, ext_id, &ext_state, &direction);
					printf("%x \n", ext_state);
					return retval;
				}
				// Control
				...
				retval = rsp1461_extension_set_pin_state(tsc_fd, ext_id, ext_pin_state_enum);
				return retval;
			}
			return(0);
		}
...
// --- LED ---
			...
			if(!strcmp("on", c->para[1])){
				retval = rsp1461_led_turn_on(tsc_fd, led);
				return retval;
			}
			// Off
			else if(!strcmp("off", c->para[1])){
				retval = rsp1461_led_turn_off(tsc_fd, led);
				return retval;
			}
			else {
			    printf("Bad arguments -> usage:\n");
			...
			if (c->cnt == 3){
				...
				if(!strcmp("status", c->para[1])){
					if (sfp_id == 7){
						...
						for (i = 0; i < 7; i++){
							retval = rsp1461_sfp_status(tsc_fd, i, &sfp_status);
				...
				if(!strcmp("control", c->para[1])){
					...
					retval = rsp1461_sfp_control(tsc_fd, sfp_id, sfp_enable, sfp_rate);
					return retval;
				}
				else {
				    printf("Bad parameter -> usage:\n");
